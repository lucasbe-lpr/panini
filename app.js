...
function buildExportText(items) {
  // Groupe par préfixe de l'ID (lettres avant les chiffres)
  const byPrefix = {};
  items.forEach(s => {
    const id = s.id;
    const match = id.match(/^([A-Za-z]+)(\d+)$/);
    const prefix = match ? match[1] : id; // si pas de chiffres, on prend l'ID entier
  ...
  Object.entries(byPrefix)
    ...
    .forEach(([prefix, ids]) => {
      // Extraire les numéros ou conserver l'ID si non numérique
      const nums = ids.map(id => {
        const match = id.match(/^[A-Za-z]+(\d+)$/);
        return match ? parseInt(match[1]) : id;
      });
      // Tri numérique pour les nombres, sinon alphabétique
      nums.sort((a, b) => {
