...
async function loadData() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`HTTP ${response.status} — fichier introuvable`);
    ...
    errorText.textContent =
      `❌ Erreur : ${err.message}. Vérifie que stickers.json est présent et que tu utilises un serveur HTTP (ex: Live Server).`;
 ...
 * ✅ Correction : plus de doublons (Set), un seul numéro par vignette
 */
